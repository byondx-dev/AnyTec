...
// Shader Chunks - Safely accessed and interpolated
const fogParsV = THREE.ShaderChunk.fog_pars_vertex ?? "";
const fogV = THREE.ShaderChunk.fog_vertex ?? "";
const fogParsF = THREE.ShaderChunk.fog_pars_fragment ?? "";
const fogF = THREE.ShaderChunk.fog_fragment ?? "";

// --- SHADERS DEFINED WITH BACKTICKS AND INTERPOLATION CHECKS ---
...
// --- SHADER VALIDATION ---
[carLightsFragment, carLightsVertex, sideSticksVertex, sideSticksFragment, roadBaseFragment, roadVertex].forEach((shader, i) => {
    if (shader.includes('${') || shader.includes('undefined')) {
        throw new Error(`Shader source #${i} not interpolated or contains undefined`);
    }
});
...
class CarLights {
    ...
    init() {
        ...
        const vertexShader = carLightsVertex
            .replace('#include <getDistortion_vertex>', typeof options.distortion === 'object' ? options.distortion.getDistortion : '')
            .replace('#define USE_FOG', ''); // Ensure no duplicate defines

        const material = new THREE.ShaderMaterial({
            fragmentShader: carLightsFragment.replace('#define USE_FOG', ''),
...
class LightsSticks {
    ...
    init() {
        ...
        const vertexShader = sideSticksVertex
            .replace('#include <getDistortion_vertex>', typeof options.distortion === 'object' ? options.distortion.getDistortion : '')
            .replace('#define USE_FOG', '');

        const material = new THREE.ShaderMaterial({
            fragmentShader: sideSticksFragment.replace('#define USE_FOG', ''),
            vertexShader: vertexShader,
...
class Road {
    ...
    createPlane(side: number, width: number, isRoad: boolean) {
        ...
        const vertexShader = roadVertex
            .replace('#include <getDistortion_vertex>', typeof options.distortion === 'object' ? options.distortion.getDistortion : '')
            .replace('#define USE_FOG', '');

        const fragmentShader = (isRoad ? roadFragment : islandFragment).replace('#define USE_FOG', '');
